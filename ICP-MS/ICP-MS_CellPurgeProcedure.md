...
2. The ICP-MS uses argon gas for the plasma.  The argon can be turned on using the main valves of the four cylinders.
...
7. The instrument's cones, including the sample cone, skimmer cone and hyper skimmer cone, should all be inspected prior to use.  Replace any worn cone to ensure optimum beam transmission.
8. With the cones inspected and satisfactory, the plasma can be ignited.
9. Click the toggle switch located under **Plasma** in the main Syngistix window.  The plasma ignition process will initiate.
10. Allow the plasma to stabilize for 20 mins prior to any analysis, including the next steps.
<div style="page-break-after: always;"></div>
11.  The picture below shows the status screen after the plasma has been successfully ignited.
...
13. Click the **Initialize** button to establish connection to the autosampler.  The screen should look like the image below if the communication is successful.
...
1. Select the **Default.dac** Conditions file and click **Open.**
...
2. Click the **Syngistix ball** followed by open.  Locate the method **Conditioning the cell for KED mode manual** and load it.
...
<div style="page-break-after: always;"></div>
3. Click the **QID** optimization and ensure that **KED QID** is checked as shown below.
...
3. Select the **Advanced Optimize** tab and select **Co** for the Analyte.
4. Select **Maximum Intensity** under **Optimization Criteria.**
5. Enter the high flow for KED, determined earlier into the appropriate gas in the Cell Parameters.
6. After the solution has reached the spray chamber and is stable, click on the Optimize icon.
7. After the readings are completed, place the capillary in the Rinse solution.
8. The AFT Voltage reading will be entered into the Conditions file.
...
2. In the Method screen, open the method **Cell Gas Optimization.mth.**
3. For good organization, create a new Dataset for collecting DRC parameter optimization data.
4. Select the **Dataset** screen.
5. Select the Syngistix Menu and **New**. Create dataset that best describes the task you are doing. For example: DRC Optimization.
6. Select **Advanced Optimize** and select **Gas Flow.**
7. In the **Parameter Range**, use a Start Value of **0.1**, an End Value of **1.0**, and a Step Value of **0.1.**
8. In the Cell parameters, enter an **RPq** value of **0.45.**
9. Select **Ramp** under Optimization Criteria.
10. Under the **Manual** tab of the **Sample** screen, enter an appropriate name for this solution: For example: **Matrix Blank Cell Gas A.** It is a good practice that this name created is descriptive. It will be used later when retrieving data. Make sure the **Write Data to Dataset** is checked.
>**Note:** **DO NOT** select any of the Analyze buttons on this screen.
11. Return to the **Advanced Optimize** tab.  Start the optimization routine by clicking on the **Optimize** icon.
12. When the routine is done, aspirate the **NexION Setup solution**.
13. Under the **Manual** tab of the **Sample** Screen, enter an appropriate name for this solution: For Example: **Matrix Spike Cell Gas A.**
>**Note:** **DO NOT** select any of the Analyze buttons on this screen.
...
>**Note:** **DO NOT** select any of the Analyze buttons on this screen.
>1. Return to the **Advanced Optimize** tab.  Start the optimization routine by clicking **Optimize.**
1. When the routine is done, aspirate the **O<sub>2</sub> Performance Check Solution.**
2. Under the **Manual** tab of the **Sample** Screen, enter an appropriate name for this solution: For Example: **Matrix Spike.**
>**Note:** **DO NOT** select any of the Analyze buttons on this screen.
...
2. When the procedure is complete, verify the values meets specifications.

...
4. To view the UV, complete the following:
  ...
  1. Select the peak algorithm you want to use by using its dropdown arrow.  Use the Peak Detect list on the Identification page to specify the type of peak detection algorithm (ICIS, Genesis, or Avalon) to use to analyze raw data. These algorithms also apply smoothing, construct a chromatogram using the scan or mass filters, assign peak numbers, generate a peak list, and determine the peak start and peak end points. All algorithms provide component peak detection and chromatographic peak detection. The ICIS and Genesis algorithms are used for MS data. **The Avalon algorithm is used for PDA, UV, and analog data**.
  1. Click "OK."
